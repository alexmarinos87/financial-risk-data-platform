...
from src.orchestration.run_portfolio_attribution import run_portfolio_attribution
...
def test_portfolio_attribution_reads_portfolio_returns_and_replays_snapshot(
    tmp_path: Path,
) -> None:
    ...
    first = run_portfolio_attribution(
        portfolio_id="us-tech-equal",
    ...
    second = run_portfolio_attribution(
        portfolio_id="us-tech-equal",
        portfolio_config_path=portfolio_config_path,
        end_date=date(2026, 1, 4),
        covariance_window=3,
    ...
    assert (
        first["curated_output"]["portfolio_risk_attribution"]["records_written"]
        == 1
    )
    assert (
        second["curated_output"]["portfolio_risk_attribution"]["records_written"]
        == 0
    )
