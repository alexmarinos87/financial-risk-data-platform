...
def check_worker_attempt_attribution_contract(
    ...
    with connection.transaction(force_rollback=True):
        ...
        def candidate(index: int, *, kind: str = "initial", request: str | None = None) -> dict[str, Any]:
            ...
            cursor.execute(
                ...
                "FROM risk_platform.portfolio_risk_notification_outbox o LIMIT 1 "
                "ON CONFLICT DO NOTHING", (event_id, event_id),
            )
        ...
        for table in (CONTEXT_TABLE, ATTRIBUTION_TABLE):
            for statement in (f"UPDATE {table} SET worker_id = worker_id WHERE worker_id = %s",
                ...
                _reject(connection, lambda sql=statement: cursor.execute(sql, (worker_id,)), "append-only")
    ...
    cursor.execute("SELECT COUNT(*) FROM risk_platform.portfolio_risk_notification_outbox WHERE event_id LIKE %s", (prefix + "%",))
    if cursor.fetchone() != (0,):
