...
current daily-return versions
  ...
  -> latest-window covariance, correlation and volatility attribution
```

The pure portfolio calculation is owned by `src/analytics/portfolio_risk.py`.
The local runner is `src/orchestration/run_portfolio_risk.py`. Covariance and
Euler volatility attribution are owned by
`src/analytics/portfolio_attribution.py` and
`src/orchestration/run_portfolio_attribution.py`. The warehouse contract is in
...
Calculate the latest covariance, correlation and component-volatility snapshot
from the retained portfolio returns:
...
That run writes `.demo/portfolio-attribution-summary.json` and one replay-safe
record under `data/curated/portfolio_risk_attribution`. See
`docs/portfolio-attribution.md` for formulas and validation rules.
...
The attribution runner applies the same current-version ordering to
`portfolio_daily_returns` after filtering to the fingerprint generated by the
selected portfolio definition.
...
The attribution snapshot uses the latest configured window of current component
returns. It persists the annualised sample covariance matrix, Pearson correlation
matrix and Euler component contributions to annualised portfolio volatility.
Those component contributions reconcile to total portfolio volatility within a
strict numerical tolerance.

The calculations are transparent portfolio demonstrations, not validated
...
| `portfolio_risk_attribution` | One latest-window covariance, correlation and component-volatility snapshot for a portfolio definition and covariance window. | Deterministic calculation ID from the definition, weighting method, window, annualisation basis and ordered current portfolio-return calculations. |
...
weight change or window change creates a distinguishable version.
...
The new attribution dataset remains curated Parquet in this increment. A
PostgreSQL attribution history table and matrix/contribution serving views are a
separate warehouse-owned change.
...
The attribution unit and integration tests additionally verify matrix symmetry,
sample annualisation, correlation null handling, current-version selection,
Euler reconciliation and replay-safe snapshot publication.

## Safety Bounds

The portfolio-return reader:
...
The attribution reader applies the same file, byte and row bounds to
`portfolio_daily_returns`, filters to one definition fingerprint and performs no
provider request.
...
Historical attribution snapshots for every event date, PostgreSQL attribution
serving, shrinkage or exponentially weighted covariance estimators, factor
models, marginal VaR, FX conversion, non-daily rebalancing schedules,
transaction costs, short positions, leverage, production scheduling, dashboards
and alert delivery remain separate decisions.
